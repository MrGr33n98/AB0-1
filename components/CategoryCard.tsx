...
export default function CategoryCard({ category, className = "" }: CategoryCardProps) {
  ...
  // Ajustando dados vindos da API
  const displayData = {
    id: category?.id,
    name: category?.name || 'Nome da Categoria',
    description: category?.short_description || category?.description || 'Explore nossos produtos e serviços de alta qualidade.',
    companiesCount: category?.companies?.length || 0,
    banner_url: !imageError && category?.banner_url
      ? category.banner_url
      : "/images/category-placeholder.jpg",
    seo_url: category?.seo_url || `categoria-${category?.id}`
  };
  ...
  return (
    ...
    >
      <Link 
        href={`/categories/${displayData.seo_url}`} 
