...
ActiveAdmin.register PendingChange do
  ...
  index do
    ...
    column :status do |pc|
      case pc.status
      when 'pending'
        status_tag 'Pendente', :warning
      when 'approved'
        status_tag 'Aprovado', :ok
      when 'rejected'
        status_tag 'Rejeitado', :error
      end
    end
    column :created_at
    actions
  ...
  show do
    ...
    panel 'Ações' do
      if pending_change.status == 'pending'
        form action: approve_admin_pending_change_path(pending_change), method: :post do |f|
          f.input :authenticity_token, type: :hidden, value: form_authenticity_token
          f.input :submit, type: :submit, value: 'Aprovar Alteração', 
                  class: 'button', style: 'background: #5bb75b'
        end

        form action: reject_admin_pending_change_path(pending_change), method: :post, 
             style: 'margin-top: 10px' do |f|
          f.input :authenticity_token, type: :hidden, value: form_authenticity_token
          f.text_area :rejection_reason, placeholder: 'Motivo da rejeição...', 
                      style: 'width: 100%; margin-bottom: 10px'
          f.input :submit, type: :submit, value: 'Rejeitar Alteração', 
                  class: 'button', style: 'background: #da4f49'
        end
      else
