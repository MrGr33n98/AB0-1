...
export function useCompany(companyId: string): UseCompanyReturn {
  ...
  const updateCompany = useCallback(
    async (data: Partial<Company>): Promise<ApiResponse> => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/company_dashboard/update_info`,
          {
            method: 'POST',
            headers: {
...
export function useDashboardStats(companyId: string) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/company_dashboard/stats?company_id=${companyId}`
      );
...
export function useNotifications(companyId: string) {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/company_dashboard/notifications?company_id=${companyId}`
      );
