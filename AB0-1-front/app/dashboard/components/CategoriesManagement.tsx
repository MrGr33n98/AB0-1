...
export default function CategoriesManagement({ companyId }: CategoriesManagementProps) {
  ...
  const fetchCategories = async () => {
    ...
    setCategories([
      { id: '1', name: 'Arquitetura e Design', status: 'active', featured: true, seo_url: 'arquitetura-design' },
      { id: '2', name: 'Construção Civil', status: 'active', featured: false, seo_url: 'construcao-civil' },
      { id: '3', name: 'Decoração', status: 'pending', featured: false, seo_url: 'decoracao' },
    ]);
    setLoading(false);
  };
  ...
  const fetchAvailableCategories = async () => {
    ...
    setAvailableCategories([
      { id: '4', name: 'Paisagismo', status: 'active', featured: false, seo_url: 'paisagismo' },
      { id: '5', name: 'Iluminação', status: 'active', featured: false, seo_url: 'iluminacao' },
      { id: '6', name: 'Automação', status: 'active', featured: false, seo_url: 'automacao' },
    ]);
  };

  const handleAddCategories = async () => {
    // Add selected categories (will be pending approval)
    const categoriesToAdd = availableCategories.filter(cat => selectedCategories.includes(cat.id));
    setCategories([...categories, ...categoriesToAdd.map(cat => ({ ...cat, status: 'pending' as const }))]);
