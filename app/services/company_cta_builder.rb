...
class CompanyCtaBuilder
  ...
  CONTEXTS = %w[card detail]
  
  # Tipos de CTAs suportados
  CTA_TYPES = %w[whatsapp phone email website custom]
  ...
  def meets_conditions?(conditions)
    ...
    if conditions['page'].is_a?(Array)
      return false unless conditions['page'].include?(@context)
    end
  ...
  def build_custom_url(url, use_utm)
    ...
    if use_utm && @utm.present?
      ...
      params['utm_source'] = @utm[:source] || @company.cta_utm_source if @utm[:source].present? || @company.cta_utm_source.present?
      params['utm_medium'] = @utm[:medium] || @company.cta_utm_medium if @utm[:medium].present? || @company.cta_utm_medium.present?
      params['utm_campaign'] = @utm[:campaign] || @company.cta_utm_campaign if @utm[:campaign].present? || @company.cta_utm_campaign.present?
